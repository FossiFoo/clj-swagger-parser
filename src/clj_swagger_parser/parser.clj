    ...
    (if (allowed-content-types content-type)                ; TODO could be checked on initialization of ring handler chain
      (if-let [deserialize-fn (second (first (filter (fn [[pattern _]] (re-matches pattern content-type)) supported-content-types)))]
        (try
          ...
          (catch Exception e
            (throw-error 400 (str "Body not parsable with given content type.")
                             {:content-type content-type
                              :error        (str e)})))
        ; if we cannot deserialize it, just forward it to the executing function
        (:body request))
      (throw-error 406 "Content type not allowed."
                       {:content-type          content-type
...
(defn parse-swagger
  [{:keys [parsers]} swagger request]
  (->> (get parsers swagger)
       (map (fn [parser] (parser request)))             ; execute all parsers of the request
       (group-by first)                                 ; group by "in"
       (map (fn [[parameter-in parameters]]             ; restructure to resemble the grouping: map[in][name] = value
